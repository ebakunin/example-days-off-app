...
export class DaysOffManagementComponent implements AfterViewInit, OnDestroy, OnInit {
    ...
    @Input() set closedDates(dates: Date[]) {
        ...
        this.#closedDates
            ...
            .forEach((datetime) => this.#closedDatesBucket.add(datetime));
    }
    ...
    @Input() set daysOff(dates: Date[] | null) {
        ...
        if (dates.length > 0) {
            ...
            this.#daysOff
                .map((date) => date.getTime())
                .forEach((datetime) => this.existingExceptionsBucket.add(datetime));
        }
    }
    ...
    readonly yearRange = `${this.todaysYear}:${this.todaysYear + 2}`;
     ...
     */
    existingExceptionsBucket = new Set<number>();
    markedForDeletionBucket = new Set<number>();
